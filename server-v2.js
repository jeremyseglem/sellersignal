...
// ===================
// LAYERED RESEARCH v2 — 12 searches in parallel
// ===================
async function gatherSearchResultsV2(ownerName, streetAddress, city, state) {
  ...
  const [propertyResults, ownerResults, intentResults] = await Promise.all([
    ...
    // LAYER 2: Owner Identity
    searchBatch([
      { label: 'LinkedIn', query: `"${firstName} ${lastName}" "${city}" ${state} site:linkedin.com` },
      { label: 'People Search', query: `"${ownerName}" "${city}" ${state} age address` },
      { label: 'Business Records', query: `"${ownerName}" "${city}" ${state} business owner LLC company` },
      { label: 'Professional', query: `"${ownerName}" "${city}" ${state} career employer` },
    ]),
    // LAYER 3: Intent & Life Signals
    searchBatch([
      { label: 'Social Media', query: `"${ownerName}" "${city}" site:facebook.com OR site:nextdoor.com` },
      { label: 'Life Events', query: `"${ownerName}" "${city}" ${state} retired OR retirement OR divorce OR obituary` },
      { label: 'Relocation', query: `"${ownerName}" "moving" OR "relocated" OR "new home" OR "downsizing"` },
      { label: 'News', query: `"${ownerName}" "${city}" ${state} news article` },
    ])
  ]);

  console.log(`  All searches complete in ${((Date.now() - startTime) / 1000).toFixed(1)}s`);
  return { propertyResults, ownerResults, intentResults };
}
...
app.post('/api/beta-research', async (req, res) => {
  ...
  try {
    ...
    const systemPrompt = `You are SellerSignal's Deep Signal research engine. You produce intelligence reports for real estate professionals.

CRITICAL RULES:
1. Every data point must be classified: CONFIRMED (directly in results), CORROBORATED (2+ sources agree), INFERRED (reasonable conclusion from facts), or NOT FOUND.
2. NEVER fabricate. If not in the search results, say "Not found". Agents use this in real conversations — wrong info destroys trust.
3. Scripts must reference specific confirmed facts. Generic scripts are labeled as generic.
4. Personality insights ONLY from confirmed professional/life data. No horoscopes.

SCORING: Start at ${preliminaryScore || 35} (from parcel data). Adjust ONLY for confirmed findings:
- Confirmed recent purchase (<3yr): -15
- Confirmed young family: -20
- Confirmed renovation: -10  
- Confirmed long ownership (15+yr) + age 60+: +20
- Confirmed retirement: +15
- Confirmed life transition: +15
- Confirmed out-of-state owner: +15
- Confirmed multiple properties: +10
- No adjusting data found: keep preliminary score, note "Limited data"
...
Return ONLY valid JSON:
{
  ...
  "score": 0-100,
  "scoreLabel": "High/Medium/Low Likelihood",
  "scoreBasis": "What confirmed facts drove this score",
  "metrics": {
    "estimatedValue": "$XXX or Not found",
    "estimatedEquity": "$XXX or Not found",
    "ownedSince": "YYYY or Not found",
    "ageRange": "XX-XX or Not found"
  },
  "confirmedFacts": ["Each confirmed fact with source type"],
  "whoTheyAre": {
    "spouse": "Name or Not found",
    "occupation": "Title at Company or Not found",
    "ownership": "How title is held or Not found",
    "decisionStyle": "Based on confirmed data or Not enough data"
  },
  "howTheyThink": {
    "financialMindset": "Based on confirmed career or Not enough data",
    "communication": "Based on confirmed context or Not enough data",
    "socialPosition": "Based on confirmed involvement or Not enough data",
    "bestChannel": "Letter/Phone/Email/Door with reasoning"
  },
  "whatMakesThemTick": {
    "personalityType": "Based on facts or Insufficient data",
    "decisionSpeed": "Based on facts or Insufficient data",
    "lifeStage": "Based on confirmed age/career/family or Insufficient data",
    "motivators": "Based on confirmed situation or Generic (labeled)",
    "pridePoints": "Based on confirmed achievements or Not enough data",
    "concerns": "Based on confirmed situation or Common concerns (labeled)"
  },
  "wealthIndicators": {
    "incomeLevel": "Based on confirmed occupation or Not enough data",
    "netWorthEstimate": "Based on confirmed facts or Not enough data",
    "evidence": ["Only evidence actually found"],
    "financialSophistication": "Low/Medium/High with reasoning or Not enough data"
  },
  "signals": [{"text": "Signal", "type": "positive/negative/neutral", "confidence": "Confirmed/Inferred"}],
  "approach": {
    "opening": "Personalized if data supports it, or generic template labeled as such",
    "keyMessages": "Based on confirmed situation",
    "avoid": "Based on confirmed sensitivities or general best practices",
    "timing": "Based on confirmed life stage or general recommendation"
  },
  "scripts": {
    "letter": "Personalized using confirmed details. [AGENT_NAME] [AGENT_PHONE] placeholders.",
    "phone": "Phone script using confirmed details.",
    "door": "Door script using confirmed details.",
    "email": "Subject + body using confirmed details."
  }
}`;

...
async function gatherSearchResults(ownerName, streetAddress, city, state, fullAddress) {
  ...
  const searches = [
    ...
    { label: 'County Records', query: `"${streetAddress}" "${city}" ${state} county property records` },
    { label: 'Owner at Address', query: `"${ownerName}" "${streetAddress}" ${city}` },
    { label: 'Owner Location', query: `"${ownerName}" "${city}" ${state}` },
    { label: 'Owner LinkedIn', query: `"${ownerName}" ${city} ${state} site:linkedin.com` },
    { label: 'Owner Business', query: `"${ownerName}" ${city} ${state} business OR company OR owner` },

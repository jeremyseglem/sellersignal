...
async function gatherSearchResults(ownerName, streetAddress, city, state) {
  ...
  const searches = [
    { label: 'Zillow Property', query: `${streetAddress} ${city} zillow` },
    { label: 'Redfin Property', query: `${streetAddress} ${city} redfin` },
    { label: 'Owner + Location', query: `"${ownerName}" ${city} ${state}` },
    { label: 'Owner LinkedIn', query: `${ownerName} linkedin ${city}` },
    { label: 'Property Records', query: `${streetAddress} ${city} property records owner` }
  ];
...
Return ONLY valid JSON:
{
    ...
    "metrics": {
        "estimatedValue": "$XXX,XXX (source) or Not found",
        "estimatedEquity": "$XXX,XXX estimated or Not found",
        "ownedSince": "YYYY (source) or Not found",
        "ageRange": "XX-XX or Not found"
    },
    "whoTheyAre": {
        "spouse": "Name if found in results, or Not found",
        "occupation": "Job title at Company (from LinkedIn/search) or Not found",
        "ownership": "How title is held if found",
        "decisionStyle": "Based on profession if known"
    },
    "howTheyThink": {
        "financialMindset": "Based on property and career data",
        "communication": "Based on profession",
        "socialPosition": "Based on findings",
        "bestChannel": "Letter/Phone/Email/Door"
    },
    "signals": ["List specific findings from search results"],
    "approach": {
    ...
    "scripts": {
        "letter": "Personalized letter using facts from search results. Use [AGENT_NAME] and [AGENT_PHONE] placeholders.",
        "phone": "Phone script based on findings.",

...
app.post('/api/deep-signal-lite', async (req, res) => {
  ...
  try {
    ...
    // Cap at 50 per request to control cost
    const batch = candidates.slice(0, 50);
    console.log(`Deep Signal Lite: scoring ${batch.length} candidates`);
    const startTime = Date.now();
    
    // Build compact candidate descriptions
    const candidateList = batch.map((c, i) => {
      return `[${i+1}] ${c.ownerName || 'Unknown owner'} — ${c.address || 'No address'}
  Type: ${c.propType || 'Unknown'} | Value: $${(c.totalValue||0).toLocaleString()} | Building: $${(c.buildingValue||0).toLocaleString()} | Land: $${(c.landValue||0).toLocaleString()}
  Acres: ${c.acres||'?'} | Absentee: ${c.isAbsentee?'Yes':'No'} | Out-of-state: ${c.isOutOfState?'Yes':' No'} | Owner state: ${c.ownerState||'local'}
  Entity: ${c.isEntity?'Yes':'No'} | Trust: ${c.isTrust?'Yes':'No'} | Estate/Heirs: ${c.isEstate?'Yes':'No'}
  Multi-property: ${c.multiCount||1} local parcels | Tenure: ${c.tenureYears != null ? c.tenureYears + ' years' : c.tenureLongTerm ? '3+ years (no recent sale)' : 'unknown'}
  Mailing address: ${c.ownerAddress ? 'Yes' : 'No'}`;
    }).join('\n\n');
    ...
    const response = await anthropic.messages.create({
      ...
      max_tokens: 4096,
      temperature: 0,
      system: `You are SellerSignal's ranking engine. Score real estate property owners on their likelihood to sell.

For each candidate, return ONLY these scores (0-100):
- sellerLikelihood: How likely is this owner to sell in the next 6-24 months? Based on transition signals, tenure, burden, life-stage mismatch. NOT just entity/trust status.
- offMarketReceptivity: If they sell, would they prefer discreet/direct/off-market? Entity, trust, luxury, privacy, acreage, absentee signals matter here.
- actionability: Can an agent reasonably reach and work this prospect? Named human with mail path = high. Opaque LLC = low.
- confidence: How much evidence supports the assessment? More data points and consistent signals = higher.
- topReason: One sentence explaining the strongest sell signal.
- bestNextMove: One of: "Call first" | "Mail first" | "Research first" | "Watch"

CRITICAL RULES:
- Entity/LLC ownership alone is NOT strong seller likelihood. It belongs in offMarketReceptivity.
- Long tenure (10+ years) IS a strong seller likelihood signal.
- Recent transfer (< 3 years) is a strong NEGATIVE for seller likelihood.
- Heirs/deceased/estate are the strongest positive seller likelihood signals.
- Trust + absentee is stronger than trust alone.
- Named individual with mailing address should score higher on actionability than anonymous LLC.
- Do NOT let every candidate score 50. Differentiate strongly. Some should be 20, some should be 80.

Return ONLY a JSON array. No explanation. No markdown. Example:
[{"id":1,"sellerLikelihood":65,"offMarketReceptivity":72,"actionability":58,"confidence":61,"topReason":"Estate with heir transition and 15+ year hold","bestNextMove":"Mail first"},...]`,
      messages: [{
        role: 'user',
        content: `Score these ${batch.length} property owner candidates:\n\n${candidateList}\n\nReturn ONLY the JSON array.`
      }]
    });

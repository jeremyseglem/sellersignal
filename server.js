...
function patternMatchFallback(name, results) {
    ...
    for (const r of results) {
        ...
        // NEW: additional pattern matches
        if (lastName && snippets.includes(lastName)) {
...
app.post('/api/life-event-scan', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      // Query 1: obituary / death / probate / estate
      allQueries.push({
        label: `life_${i}_obit`,
        candidateIdx: i,
        type: 'obituary',
    ...
    const CLAUDE_EXTRACTION_CAP = 20; // only top 20 get full LLM extraction
...

{
  "signals": [
    {
      "type": "one of: obituary, probate, divorce, retirement, employment_change, relocation, business_dissolution, bankruptcy, care_facility, empty_nester, health_issue, inheritance, listing_failed, price_reduced, recently_sold, age_indicator, financial_distress, lifestyle_change, family_change",
...
app.post('/api/label/check-transfers', async (req, res) => {
  ...
  try {
    ...
    const { data: snapshots, error } = await supabase
      ...
      .select('parcel_id, snapshot_date, source_key, owner_name_raw, zip_code, tier, seller_likelihood, owner_type, tenure_years, is_absentee, is_out_of_state')
    ...
    for (let i = 0; i < toCheck.length; i += 5) {
      ...
      const results = await Promise.allSettled(batch.map(async (snap) => {
        // Parse lat/lng from parcel_id if stored as "lat-lng"
  ...
  try {
    ...
    for (let i = 0; i < toCheck.length; i += 5) {
      ...
      const results = await Promise.allSettled(batch.map(async (snap) => {
        ...
        if (!lat || !lng || !snap.source_key) return { snap, currentOwner: null };
        const currentOwner = await queryCurrentOwner(snap.source_key, lat, lng);
        return { snap, currentOwner };
      }));

...
app.post('/api/deep-signal-lite', async (req, res) => {
  ...
  try {
    ...
    console.log(`Deep Signal Lite: scoring ${batch.length} evidence bundles`);
    const startTime = Date.now();
...
YOUR ROLE: Validate and lightly adjust the base scores. You are a refinement layer, not the primary scorer.

BOUNDED REFINEMENT RULES:
- You may adjust any score by AT MOST +/- 8 points from the base score
- Exception: if you identify a CLEAR factual reason the base score is wrong (e.g., the base missed a strong blocker, or the base penalized an estate that clearly has heirs), you may adjust up to +/- 15
...
app.post('/api/life-event-scan', async (req, res) => {
  ...
  try {
    ...
    const batch = candidates.slice(0, 100); // cap at 100
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      // Query 1: obituary / death / probate
      allQueries.push({
        label: `life_${i}_obit`,
        candidateIdx: i,
        type: 'obituary',
        query: `"${lastName}" ${city} ${state} obituary OR death OR probate 2024 2025 2026`
      });
      
      // Query 2: listing history / business dissolution / retirement
      if (address) {
        allQueries.push({
        ...
        allQueries.push({
          ...
          candidateIdx: i,
          type: 'business',
          query: `"${name}" ${state} dissolved OR inactive OR retired OR relocation`
        });
      }
    }
    ...
    // Pattern match on search snippets — no LLM needed
    const events = [];
    for (let i = 0; i < batch.length; i++) {
      const c = batch[i];
      const results = searchResults[i] || [];
      const found = [];
      const name = (c.ownerName || '').toUpperCase();
      const lastName = name.split(/[\s,]+/).filter(Boolean).pop() || '';
      
      for (const r of results) {
        const snippets = (r.results || []).map(x => `${x.title || ''} ${x.snippet || ''}`).join(' ').toUpperCase();
        
        if (r.type === 'obituary') {
          if (lastName && snippets.includes(lastName) && /OBITUAR|PASSED\s*AWAY|DEATH\s*NOTICE|MEMORIAL|PROBATE|SURVIVED\s*BY/i.test(snippets)) {
            found.push({ type: 'obituary_match', signal: 'possible obituary/death in household — estate transition likely', confidence: 'medium' });
          }
          if (lastName && snippets.includes(lastName) && /PROBATE|ESTATE\s*OF|EXECUTOR|ADMINISTRATOR/i.test(snippets)) {
            found.push({ type: 'probate_match', signal: 'probate filing detected — forced transition', confidence: 'high' });
          }
        }
        
        if (r.type === 'listing') {
          if (/WITHDRAWN|EXPIRED|CANCELED|DELISTED|OFF\s*MARKET/i.test(snippets)) {
            found.push({ type: 'listing_failed', signal: 'prior listing attempt failed — may be open to direct approach', confidence: 'medium' });
          }
          if (/PRICE\s*REDUC|PRICE\s*CUT|PRICE\s*DROP/i.test(snippets)) {
            found.push({ type: 'price_reduced', signal: 'price reduction history — motivated seller signal', confidence: 'medium' });
          }
          if (/RECENTLY\s*SOLD|JUST\s*SOLD|SALE\s*PENDING/i.test(snippets)) {
            found.push({ type: 'recently_sold', signal: 'recent sale activity at address', confidence: 'low' });
          }
        }
        
        if (r.type === 'business') {
          if (/DISSOLV|INACTIVE|REVOKED|CANCELLED|WITHDRAWN/i.test(snippets)) {
            found.push({ type: 'business_dissolved', signal: 'business dissolution — asset simplification likely', confidence: 'medium' });
          }
          if (/RETIRE|RETIREMENT/i.test(snippets) && lastName && snippets.includes(lastName)) {
            found.push({ type: 'retirement', signal: 'retirement evidence — life-stage transition', confidence: 'low' });
          }
        }
      }
      
      if (found.length > 0) {
        events.push({ candidateIdx: i, parcelId: c.parcelId || null, events: found });
      }
    }
...
app.get('/api/cache/clear', async (req, res) => {
  ...
  if (secret !== (process.env.ADMIN_KEY || 'ss_admin_2026')) {

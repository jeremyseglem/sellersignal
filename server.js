...
app.post('/api/deep-signal-lite', async (req, res) => {
  ...
  try {
    ...
    // Build market profile context
    let marketContext = '';
    if (marketProfile) {
      marketContext = `\n\nMARKET-SPECIFIC INTELLIGENCE:\n${marketProfile}\n\nUSE THIS CONTEXT to interpret features differently for this market. A trust in Palm Beach means something completely different than a trust in Seattle. An absentee owner with a Minnesota mailing address in Phoenix is a snowbird — that's a strong signal. The same absentee pattern in Charlotte might be an investor. Score based on what these features MEAN in this specific market, not what they mean generically.`;
    }
    
    // Build market calibration context if available from backtest
    let calibrationContext = '';
    if (calibration && calibration.featureRates) {
      calibrationContext = `\n\nMARKET-SPECIFIC CALIBRATION DATA (from ${calibration.sold24mo || '?'} actual sales in ZIP ${calibration.zipCode || '?'} over 24 months):
Base turnover rate: ${calibration.baseRate24mo || '?'}%
Avg score of properties that sold: ${calibration.avgScoreSold || '?'}
Avg score of properties that didn't sell: ${calibration.avgScoreNotSold || '?'}
Feature conversion rates (actual % that sold in 24mo):
${Object.entries(calibration.featureRates).map(([k,v]) => `  ${k}: ${v}`).join('\n')}

USE THIS DATA to calibrate your adjustments for this specific market. If trusts sell at 3x the base rate, the trust signal deserves more weight. If vacant land sells below the base rate, suppress vacant-land scores. These are REAL outcomes — weight them heavily in your reasoning.`;
    }
    ...
    const candidateList = batch.map((c, i) => {
      ...
      desc += `\n  OWNER TYPE: ${obs.ownerType || 'unknown'}`;
      desc += `\n  VALUE: $${(obs.assessedValue||0).toLocaleString()} (land: $${(obs.landValue||0).toLocaleString()}, building: $${(obs.buildingValue||0).toLocaleString()})`;
      desc += `\n  PROPERTY: ${obs.propertyType || 'unknown'} | Vacant: ${obs.isVacantLand ? 'Yes' : 'No'}`;
      desc += `\n  OCCUPANCY: ${der.likelyOccupancy || 'unknown'} | Absentee: ${obs.isAbsentee ? 'Yes' : 'No'} | Out-of-state: ${obs.isOutOfState ? 'Yes' : 'No'}`;
      ...
      desc += `\n  CONTACT PATH: mail=${der.mailPathQuality || 'unknown'}, identity=${der.ownerIdentityQuality || 'unknown'}`;
      desc += `\n  BURDEN: ${der.propertyBurden || 'unknown'}`;
      
      // Evidence signals
      if (ev.timeSignals?.length) desc += `\n  TIME SIGNALS: ${ev.timeSignals.join('; ')}`;
      if (ev.transitionSignals?.length) desc += `\n  TRANSITION SIGNALS: ${ev.transitionSignals.join('; ')}`;
      ...
      desc += `\n  EVIDENCE: ${ev.totalCorroborating || 0} corroborating signals → pre-gate: ${ev.gateResult || 'unknown'} (${ev.gateReason || ''})`;
      
      // Base scores from deterministic model
      const bs = c.baseScores || {};
      desc += `\n  BASE SCORES: SL=${bs.sellerLikelihood||'?'} OMR=${bs.offMarketReceptivity||'?'} ACT=${bs.actionability||'?'} CONF=${bs.confidence||'?'}`;
      
      // Gaps and confidence
      if (gaps.length) desc += `\n  GAPS: ${gaps.join(', ')}`;
      desc += `\n  CONFIDENCE: ownership=${conf.ownership||'?'}, tenure=${conf.tenure||'?'}, contact=${conf.contact||'?'}, market=${conf.market||'?'}`;
    ...
    const response = await anthropic.messages.create({
      ...
      max_tokens: 8192,
      temperature: 0,
      system: `You are SellerSignal's evidence-based REFINEMENT engine. You do NOT score from scratch. You refine deterministic base scores using evidence reasoning.

Each candidate comes with:
- A structured evidence bundle (observed, derived, gaps, confidence, signals)
- BASE SCORES from a deterministic model (sellerLikelihood, offMarketReceptivity, actionability, confidence)
- Evidence signal counts and pre-gate tier assignment

YOUR ROLE: Validate and adjust the base scores. You are a refinement layer that corrects known systematic biases in the deterministic model.

BOUNDED REFINEMENT RULES:
- Default adjustment range: +/- 15 points from the base score
- If you identify a SYSTEMATIC BIAS in the base scoring (see known biases below), you may adjust up to +/- 25 points with explicit reasoning
- You MUST state the base score and your adjusted score for transparency
- If the base score looks reasonable, return it unchanged or with minimal adjustment
- NEVER cluster all scores around 50. If the base has spread, preserve the spread.

KNOWN SYSTEMATIC BIASES TO CORRECT:
- Owner-occupants (individual, not absentee, improved property) are UNDER-SCORED by the base model. A named individual with 15+ year tenure, age 60+, and a usable contact path deserves SL 40-55, not 20-30.
- Vacant land with absentee+out-of-state is OVER-SCORED relative to improved residential property. An out-of-state vacant lot is less actionable than a named homeowner with life-event evidence.
- Entity type alone (trust, LLC) is not evidence of seller intent. The base model gives +8 to +16 for entity type without requiring timing evidence. Correct this: entities without time/transition signals should have SL suppressed toward 25-35.

EVIDENCE REASONING:
- Ownership type alone (trust, LLC, entity) is NOT seller likelihood. Do not boost SL for entity type without time/transition evidence.
- Missing tenure MUST suppress confidence. If tenure confidence is "none", your confidence score cannot exceed base + 5.
- If evidence is thin (1 signal, major gaps), "Research first" or "Watch" is the correct bestNextMove.
- Blockers should be respected aggressively.
- Life-event signals (obituary, retirement, divorce, relocation) are the STRONGEST evidence — weight them heavily.
- Age 60+ combined with long tenure is a meaningful positive signal for seller likelihood.
- A confident "insufficient evidence" is more valuable than an inflated score.
${marketContext}${calibrationContext}

OUTPUT per candidate:
{
  "id": 1,
  "sellerLikelihood": adjusted 0-100,
  "offMarketReceptivity": adjusted 0-100,
  "actionability": adjusted 0-100,
  "confidence": adjusted 0-100,
  "topReason": "One sentence: what the evidence COLLECTIVELY suggests about this seller's situation",
  "bestNextMove": "Call first|Mail first|Research first|Watch",
...
Return ONLY a JSON array. No explanation. No markdown.`,
      messages: [{
        role: 'user',
        content: `Refine the base scores for these ${batch.length} candidates. Each has deterministic base scores — adjust within the bounded range based on evidence reasoning:\n\n${candidateList}\n\nReturn ONLY the JSON array.`
      }]
    });

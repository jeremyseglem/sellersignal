...
function normalizeOwnerName(rawName) {
  ...
  if (/LLC|TRUST|LTD|PARTNERSHIP|INC|CORP|ESTATE|FOUNDATION|COMPANY|HOLDINGS|GROUP|RANCH|FARM|PROPERTIES|INVESTMENTS|ASSOCIATES|VENTURES|ENTERPRISES|PARTNERS|DEVELOPMENT|DEVELOPERS|REALTY|MANAGEMENT/i.test(rawName)) {
    return rawName.trim();
  }
...
function buildDeepSignalPrompt(propertyAddress, preliminaryScore, city) {
  return `You are SellerSignal's Deep Signal research engine. You produce actionable intelligence reports for real estate professionals who will use this information in real conversations with property owners.

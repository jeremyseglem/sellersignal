...
// Throttled batch search — fires in groups of 8 with pauses to avoid SerpAPI rate limits
...
async function gatherSearchResultsV2(ownerName, streetAddress, city, state) {
  ...
  const allSearches = [
    ...
    // AGENT DETECTION (7)
    { label: 'Zillow Agent', query: `"${ownerName}" ${city} ${state} site:zillow.com/profile` },
    ...
    { label: 'RE Agent General', query: `"${ownerName}" "${city}" realtor OR "real estate agent" OR broker` },
    ...
    { label: 'News', query: `"${ownerName}" "${city}" ${state} news OR article` },
    { label: 'Professional Profile', query: `"${ownerName}" "${city}" ${state} professional OR career OR work` },
    
    // FAMILY + RECORDS (3)
    { label: 'Family', query: `"${ownerName}" "${city}" spouse OR wife OR husband OR family` },
    { label: 'Marriage', query: `"${ownerName}" ${state} marriage OR wedding` },
    ...
    { label: 'Community', query: `"${ownerName}" "${city}" ${state} board OR volunteer OR foundation OR donation` },
    { label: 'Other Properties', query: `"${ownerName}" ${state} property OR parcel OR deed -"${streetAddress}"` },
    { label: 'Court Records', query: `"${ownerName}" "${city}" ${state} court OR filing OR lien OR judgment` },
    { label: 'Phone Email', query: `"${ownerName}" "${city}" ${state} phone OR email OR contact` },
    { label: 'Age Records', query: `"${ownerName}" "${city}" age OR born OR birthday` },
  ];

  // Fire ALL 40 simultaneously
  ...
  return {
    ...
    connectionResults: Object.fromEntries(Object.entries(results).filter(([k]) => 
      ['Zillow Agent','Realtor.com Agent','Redfin Agent','RE Agent General','Brokerage Bio','Agent Reviews','Agent Transactions'].includes(k)))
  };
}
